...
console.log(`가장 큰 값 : ${Math.max(10, 30, 20)}`); // 30, max는 배열이 아니라 숫자를 인수로 받음
...
// 일반 값과 혼합해서 사용 가능
console.log(`가장 작은 값 : ${Math.min(1, ...arr1, 2, ... arr2, 3)}`); // 1
...
let str = 'JavaScript';
console.log([...str]); 
console.log(Array.from(str)); // 이터러블 객체 이외에도 무언가를 배열로 바꿀 떄 보편적으로 사용한다.